...
interface RetryIdempotencyContract {
  ...
  server_behavior: {
    ...
    duplicate_in_flight: string;
    conflicting_reuse: string;
    malformed_key: string;
  };
  async_completion: {
...
export async function capabilitiesCommand(flags: Record<string, string | boolean>): Promise<void> {
  ...
  console.log(`  Contract: ${RETRY_IDEMPOTENCY_CONTRACT.id}`);
  console.log(`  Applies to: ${RETRY_IDEMPOTENCY_CONTRACT.applies_to.join(", ")}`);
  console.log(`  Initial scope: ${RETRY_IDEMPOTENCY_CONTRACT.initial_endpoint_scope.methods.join(", ")} on ${RETRY_IDEMPOTENCY_CONTRACT.initial_endpoint_scope.patterns.join("; ")}`);
  console.log(`  Header: ${RETRY_IDEMPOTENCY_CONTRACT.mutating_requests.idempotency_key_header}`);
  console.log(`  Key format: ${RETRY_IDEMPOTENCY_CONTRACT.mutating_requests.key_format}`);
  ...
  console.log(`  Retention: ${RETRY_IDEMPOTENCY_CONTRACT.server_behavior.retention_window}`);
  console.log(`  Duplicate success: ${RETRY_IDEMPOTENCY_CONTRACT.server_behavior.duplicate_success}`);
  console.log(`  Duplicate in-flight: ${RETRY_IDEMPOTENCY_CONTRACT.server_behavior.duplicate_in_flight}`);
  console.log(`  Conflicting reuse: ${RETRY_IDEMPOTENCY_CONTRACT.server_behavior.conflicting_reuse}`);
